...
interface RetailerCountCardProps {
  data?: {
    ...
    // New dynamic city distribution (preferred)
    cityDistribution?: Array<{ city: string | null; count: number }>;
    // Backwards compat: legacy regional distribution
...
export function RetailerCountCard({ data }: RetailerCountCardProps) {
  ...
  // Build a dynamic list of city stats. Prefer `cityDistribution` if provided, else fall back to legacy regions.
  const cityDistribution = data?.cityDistribution && data.cityDistribution.length > 0
    ? // map city names and counts
      data.cityDistribution.map((c, i) => ({ city: c.city || 'Unknown', count: c.count, color: ['from-electric-blue to-electric-cyan','from-electric-green to-electric-blue','from-electric-purple to-electric-pink','from-electric-orange to-electric-pink'][i % 4] }))
    : // fallback to regions
      [
        { city: 'North', count: regionalDistribution.north, color: 'from-electric-blue to-electric-cyan' },
        { city: 'South', count: regionalDistribution.south, color: 'from-electric-green to-electric-blue' },
        { city: 'East', count: regionalDistribution.east, color: 'from-electric-purple to-electric-pink' },
        { city: 'West', count: regionalDistribution.west, color: 'from-electric-orange to-electric-pink' },
      ]
        ...
        <div className="space-y-4">
          ...
          <div className="space-y-3">
            ...
            <div className="grid grid-cols-2 gap-3">
              {cityDistribution.map((stat, index) => (
                <div
                  key={index}
                  className="p-3 rounded-lg bg-gradient-to-r from-white/20 to-gray-300/90 dark:from-gray-800/50 dark:to-gray-700/50 backdrop-blur-sm"
                >
                  <div className="flex items-center justify-between">
                    <span className="text-sm font-medium text-gray-700 dark:text-gray-300">{stat.city}</span>
